                ...
                <div id="comment-<?php comment_ID(); ?>">
                    <h4 class="media-heading clearfix">
                        ...
                        <strong class="comment-reply pull-right">
                            <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Responder', 'omega-td' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
                        </strong>
                    </h4>
                    <?php comment_text(); ?>
                </div>
...
function oxy_comment_form( $args = array(), $post_id = null ) {
    ...
    $fields =  array(
        'author' => '<div class="form-group form-icon-group"><input id="author" name="author" placeholder="' . __('Nombre *', 'omega-td') . '" type="text" class="input-block-level form-control" value="' . esc_attr( $commenter['comment_author'] ) .  '"/><i class="fa fa-user"></i></div>',
        'email'  => '<div class="form-group form-icon-group"><input id="email" name="email" placeholder="' . __('E-mail *', 'omega-td') . '" type="text" class="input-block-level form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" /><i class="fa fa-envelope"></i></div>',
        'url'    => '',
    );

    $required_text = sprintf( ' ' . __('Los campos requeridos estan marcados como %s', 'omega-td'), '<span class="required"><a>*</a></span>' );
    $defaults = array(
        'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
        'comment_field'        => '<div class="form-group form-icon-group"><textarea id="comment" name="comment" placeholder="' . __('Tu mensaje', 'omega-td') . '" class="input-block-level form-control" rows="3"></textarea><i class="fa fa-pencil"></i></div>',
        'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'Debes iniciar sesion para  <a href="%s">logged in</a> agregar un comentario.', 'omega-td' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
        'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Iniciaste sesión como  <a href="%1$s">%2$s</a>. <a href="%3$s" title="Cerrar sesión de esta cuenta ">Cerrar sesión?</a>', 'omega-td' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
        'comment_notes_before' => '',
        'comment_notes_after'  => '',

        'id_form'              => 'commentform',
        'id_submit'            => 'submit',
        'title_reply'          => __( 'Realiza un comentario', 'omega-td' ),
        'title_reply_to'       => __( 'Responder', 'omega-td' ),
        'cancel_reply_link'    => __( 'Cancelar respuesta', 'omega-td' ),
        'label_submit'         => __( 'Comenta', 'omega-td' ),
    );
            ...
            if( !empty( $wp_query->max_num_pages ) ) : ?>
                <nav class="post-navigation text-center">
                    <ul class="pagination">
                        ...
                        <li class="mde-next-page <?php echo $paged < $wp_query->max_num_pages ? '' : 'disabled' ?>">
