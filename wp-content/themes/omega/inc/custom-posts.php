...
function oxy_fetch_custom_columns($column) {
    ...
    switch( $column ) {
        ...
        ///MDE-CASOS DE EXITO//////
        case 'mde-testimonial-theme':
            echo get_the_term_list( $post->ID, 'mde_testimonial_theme', '', ', ' );
        break;

        case 'author':
            echo '<a href="'.the_author_meta( 'user_url' ).'">'.the_author_meta( 'display_name' ).'</a>';
        break;



        case 'mde_category':
            echo get_the_term_list( $post->ID, 'mde_category', '', ', ' );
        break;
